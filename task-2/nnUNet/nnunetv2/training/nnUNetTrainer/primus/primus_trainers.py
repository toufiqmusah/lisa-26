...
from nnunetv2.training.lr_scheduler.warmup import Lin_incr_LRScheduler, PolyLRScheduler_offset, PolyLRScheduler_warm_restarts
...
class nnUNet_PrimusV3S_Trainer_100ep(nnUNet_PrimusV3S_Trainer):
    """PrimusV3S trainer configured for 100 epochs."""
...
class nnUNet_PrimusV3S_Trainer_1250ep(nnUNet_PrimusV3S_Trainer):
    """PrimusV3S trainer configured for 1250 epochs."""
    ...
    def __init__(
        ...
        super().__init__(plans, configuration, fold, dataset_json, device)
        self.num_epochs = 1250


class nnUNet_PrimusV3S_Trainer_1250ep_WarmRestarts(nnUNet_PrimusV3S_Trainer):
    """PrimusV3S trainer for 1250 epochs with PolyLR warm restarts every 250 epochs (5 cycles)."""
    ...
    def __init__(
        ...
        super().__init__(plans, configuration, fold, dataset_json, device)
        self.num_epochs = 1250
        self.restart_period = 250  # warm restart every 250 epochs
    ...
    def configure_optimizers(self, stage: str = "warmup_all"):
        ...
        if stage == "warmup_all":
            ...
            self.print_to_log_file("train whole net, PolyLR with warm restarts")
            if self.training_stage == "warmup_all":
                optimizer = self.optimizer
            else:
                optimizer = torch.optim.AdamW(
            ...
            self.print_to_log_file(
                f"Initialized train optimizer and PolyLR warm restarts scheduler "
