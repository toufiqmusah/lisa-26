...
class PolyLRScheduler_warm_restarts(_LRScheduler):
    """PolyLR schedule with periodic warm restarts.

    Within each cycle of `restart_period` steps, the LR follows poly decay:
        lr = initial_lr * (1 - step_in_cycle / restart_period) ^ exponent

    At the start of each new cycle the LR resets to initial_lr.
    An optional `start_step` offset is subtracted first (to skip warmup epochs).
    """
    ...
    def step(self, current_step=None):
        ...
        step_in_cycle = current_step % self.restart_period

        new_lr = self.initial_lr * (1 - step_in_cycle / self.restart_period) ** self.exponent
        for param_group in self.optimizer.param_groups:
            param_group["lr"] = new_lr
