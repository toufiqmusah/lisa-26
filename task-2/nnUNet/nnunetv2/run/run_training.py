...
def maybe_load_checkpoint(nnunet_trainer: nnUNetTrainer, continue_training: bool, validation_only: bool,
                          pretrained_weights_file: str = None, resample_weights: bool = False):
    ...
    if continue_training:
        ...
        if pretrained_weights_file is not None:
            if resample_weights:
                target_k = 5
                if hasattr(nnunet_trainer.__class__, 'kernel_size'):
                    nnunet_trainer.__class__.kernel_size = target_k
            if not nnunet_trainer.was_initialized:
                nnunet_trainer.initialize()
            if resample_weights:
                from nnunetv2.training.nnUNetTrainer.mednext.nnUNetTrainerMedNext import trilinear_kernel_expand
                ckpt = torch.load(pretrained_weights_file, map_location='cpu', weights_only=False)
                src = ckpt.get("network_weights", ckpt.get("state_dict", ckpt))
                expanded = trilinear_kernel_expand(src, target_kernel_size=target_k)
                model_dict = nnunet_trainer.network.state_dict()
                compat = {k: v for k, v in expanded.items() if k in model_dict}
                model_dict.update(compat)
                nnunet_trainer.network.load_state_dict(model_dict)
                n_exp = sum(
                    1 for k in compat if src.get(k) is not None
                    and src[k].shape != model_dict[k].shape
                )
                print(f"Resampled pretrained weights: {len(compat)} keys loaded, "
                      f"{n_exp} kernel-expanded ({target_k}x{target_k}x{target_k})")
            else:
...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, disable_checkpointing, c, val,
            pretrained_weights, npz, val_with_best, world_size, resample_weights=False):
    ...
    maybe_load_checkpoint(nnunet_trainer, c, val, pretrained_weights, resample_weights)
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        maybe_load_checkpoint(nnunet_trainer, continue_training, only_run_validation, pretrained_weights, resample_weights)
...
def run_training_entry():
    ...
    parser.add_argument('-resample_weights', action='store_true', required=False, default=False,
                        help='[OPTIONAL] Use together with -pretrained_weights to expand kernel weights (e.g. 3x3x3 '
                             'to 5x5x5) via trilinear interpolation. Trainer must have a kernel_size class attribute.')
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 device=device, resample_weights=args.resample_weights)
