...
\subsection{Whole-Image Radiomics Quality Assessment}
\label{sec:radiomics_qa}

As a baseline for automated quality assessment of ULF-MRI, we developed a radiomics-based pipeline operating on whole-image texture and intensity features. For each volume, 93 handcrafted features spanning six PyRadiomics families---first-order statistics, GLCM, GLRLM, GLSZM, GLDM, and NGTDM---were extracted independently from each of the three orthogonal orientations after foreground masking. Per-view variance and correlation filtering (Pearson $r > 0.95$) reduced the feature set to approximately 66 discriminative dimensions per view. Classification was performed with 21 separate XGBoost models (one per artifact-label combination per view, each with class-weighted hyperparameter tuning), and the per-view probability estimates were averaged across orientations before argmax assignment. Despite its simplicity, this approach achieved 0.798 F1-micro on the challenge validation set, establishing a strong non-learning baseline. However, the large gap between micro and macro F1 (0.463) highlighted the method's vulnerability to ordinal class imbalance, as the handcrafted features lacked the capacity to distinguish the sparse minority classes (1 and 2) from the dominant class 0 for many artifact types.

\subsection{DINOv3-Based Deep Learning Quality Assessment}
\label{sec:dinov3_qa}

To address the minority-class limitations of the radiomics baseline, we developed an end-to-end deep learning model built on a frozen DINOv3 ViT-S+ backbone (36M parameters, pretrained on LVD-1689M) adapted via low-rank LoRA adapters (rank 8) inserted into the attention projections of all transformer blocks. Each 3D volume was processed by sampling 64 axial slices at uniform stride 2 through the slice dimension, which were passed as a batch through the frozen backbone to produce per-slice CLS and patch token representations. These were concatenated (768-d per slice), processed by two trainable VisionBlocks with GELU activations and residual connections, and aggregated via a learned attention pooling layer that weights slices by their relevance to each artifact decision. A set of seven per-artifact linear heads mapped the pooled volume embedding to three-class ordinal logits, trained with focal loss ($\gamma=2$, $\alpha=0.25$) to penalize overconfident class-0 predictions. The model was trained on all 532 volumes for 150 epochs with a three-way learning rate schedule (LoRA: $1\times10^{-4}$, VisionBlocks: $1\times10^{-4}$, heads: $1\times10^{-3}$), cosine annealing to zero, and gradient scaling under mixed precision (float16). This architecture exploits the rich visual priors of DINOv3 pretraining while the attention pooling mechanism preserves sparse artifact signals that would be washed out by global averaging, and the focal loss directly counteracts the ordinal imbalance that limited the radiomics approach. The total parameter count is 32.5M, of which only 3.8M are trainable (221K in LoRA adapters, 3.5M in VisionBlocks, and 17K in the classification heads).
